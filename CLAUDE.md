...
- 每次 task 完成 / 新需求 / 章节新增删改 → **立刻 + 一定要** 同步：
  0. **`spec/` + `plan/`** ── 所有实现行为变更必须同步对应章节；不允许文档继续描述旧行为或未来态行为
...
- 每次完成逻辑节点并 `git commit` 后，必须跑一次与改动范围匹配的验证；涉及应用实现 / Tauri 配置 / 构建配置时，提交后跑 `pnpm build` / `cargo build` / `tauri dev` 中必要的一项或多项
- `pnpm install` 等依赖安装命令仍不主动跑；只有用户明确要求或本地缺依赖导致无法验证时，先说明原因再处理
...
- **progress/** 实施跟踪（5 篇对齐 Phase 1-5）
  ...
  - `03_m2_ai_settings` DeepSeek + secrets.json + 设置面板 + 自定义快捷键 + 签名公证
