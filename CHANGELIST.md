...
| 字段 | 内容 |
| --- | --- |
| 变更 | **后端 High**：ai_fix 的 HTTP 错误不再把上游完整原始 body（含 request id）透传前端，改为对齐 test_connection 的状态码 + 简短归因；DB 打开失败改为 `manage(Option<Db>)` + `require_db` 守卫，history 命令返回可读 Sqlite 错误而非 panic 假死；`string_to_json` 反转义正确处理 UTF-16 代理对（emoji 不再静默丢字符），落单代理对报 Parse 错误。**前端 High**：⌘K/⌘W 从 react-hotkeys-hook（contentEditable 下失效）改为 window capture 手工监听；两个竞争的 Cmd+Enter capture 监听器合并为单一优先级分发器（消除注册顺序脆弱性和双触发风险）；AiFixPane 加 staleness 守卫（resolve 时校验编辑器内容未变才落 store，防陈旧 diff 回填 + StrictMode 双发）。**后端 Medium**：Retina 窗口尺寸用逻辑像素（除 scale_factor）；window_set_theme 改 tokio oneshot 不阻塞 worker；全部 store 锁 `.expect` → `into_inner` 恢复（防毒化）；删每请求 `remove_var`（no_proxy 已覆盖）。**前端 Medium**：`listen().then` 泄漏改 disposed 守卫（App/FloatingWindow）；History 搜索加 180ms debounce + 请求序号守卫；`String(cause)` → `formatError` 统一错误格式化（HistoryModal/ApiKeyInput/ShortcutInput）；TabBar AI 开启时也渲染可点击 AI Fix 入口；locale 启动闪英文修复（settings 未加载前不 changeLanguage）；split 快捷键录入强制修饰键；StatusBar/TreePanel 硬编码英文迁移 i18n。**Low**：About 版本改 `getVersion()` 动态读取；死代码清理（logger.ts/EmptyState.tsx/JsonBracesIcon/runPaneApply ai-fix 死分支）；unwrap 超时报真实 ms；settings/window 序列化失败返回错误不写空文件；indentationMarkers 补 dark 色；移除 tauri.conf 遗留 `api.deepseek.com` ATS 例外域；`test:node` glob 改 `find` 防深嵌套漏跑；修复 logging/redact/capabilities 中失效的旧 spec 路径引用。**功能移除**：删 `clipboard_read` stub + `autoPasteClipboard` 设置（后端从未实现）；删 `restoreWindow` 设置字段（行为从未接线，会话驻留靠窗口隐藏而非此设置）；删 `⌘⇧L` restore-last 整条功能链 + session 子系统（save/load/clear 命令 + store 模块 + LastSession 类型 + 全局快捷键注册 + ShortcutAction::RestoreLast 变体 + 设置页 ShortcutInput 行）。**文档**：README 刷新（撤 To JSON tab、DeepSeek→双协议、pin→star、FTS5→SQLite）；TODO 迁出已完成的 P1-02/P1-04，新增 P2-01（RedactLayer 延后项）。 |
| 影响文档 | `src-tauri/src/{ai/provider,engine/stringify,engine/unwrap,cmds/window,cmds/history,cmds/system,shortcuts/mod,store/{db,settings,window,secrets},logging/{mod,redact},main,types}.rs`、`src-tauri/Cargo.toml`、`src-tauri/tauri.conf.json`、`src-tauri/capabilities/default.json`、删 `src-tauri/src/cmds/session.rs` + `src-tauri/src/store/session.rs`、`src/hooks/{useGlobalHotkeys,useDebouncedTransform}.ts`、`src/panes/AiFixPane.tsx`、`src/shell/{FloatingWindow,TabBar,StatusBar}.tsx`、`src/history/HistoryModal.tsx`、`src/settings/{SettingsView,ApiKeyInput,ShortcutInput}.tsx`、`src/editor/{extensions,transforms}.ts`、`src/ipc/{commands,error}.ts`、`src/i18n/useLocaleSync.ts`、`src/store/settings.ts`、`src/types/{enums,events,commands}.ts`、`src/components/icons.tsx`、`src/styles/global.css`、删 `src/services/logger.ts` + `src/components/EmptyState.tsx`、`src/locales/{en-US,zh-CN}/{shell,settings,panes}.json`、`tests/**`、`README.md`、`TODO.md`、`CHANGELIST.md`。 |
| 关联 | 五路并行审查 agent（Rust 后端 / 前端 TS / 产品契约 / 文档一致性 / diff 对抗评审）产出 7 High + ~15 Medium + ~20 Low；按「阻塞→High→Medium→Low→文档」有序推进，每阶段分段验证（tsc / cargo check / cargo test 43 项 / node test 66 项全绿）。功能移除项由用户审核后确认：clipboard 从未实现、restoreWindow 从未接线、restore-last/session 被 history 功能覆盖。 |
