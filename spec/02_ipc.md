...
### 3.1 为什么是单一 enum 而不是 anyhow / 每模块自带 Error
...
AI Fix 由前端本地 loading 状态驱动；IPC 层只负责返回结构化成功 / 失败 payload，具体 UI 呈现见 [design/02 § 4](../design/02_interaction.md#4-ai-fix-流程仅错误时) 与 [design/02 § 5.1](../design/02_interaction.md#5.1-错误呈现矩阵)。
...
错误类型的完整定义（variant / payload）见 [13 § 1](13_schemas.md)；本表只描述 IPC 返回与调用方可用信息。用户可见呈现由 [design/02 § 5.1](../design/02_interaction.md#5.1-错误呈现矩阵) 维护。
